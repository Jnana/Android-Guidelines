...
Braces around the statements are __always__ required even if the condition and the body fit on one line.  
...
```java
Picasso.with(context).load("http://gangverk.is/images/sexyAtli.jpg").into(imageView);
```

```java
Picasso.with(context)
        .load("http://gangverk.is/images/sexyAtli.jpg")
        .into(imageView);
```
...
```java
loadPicture(context, "http://gangverk.is/images/sexyAtli.jpg", mImageViewProfilePicture, clickListener, "Title of the picture");
```

```java
loadPicture(context,
        "http://gangverk.is/images/sexyAtli.jpg",
...
String names start with a prefix that identifies the __section they belong to__. For example `registration_email_hint` or `registration_name_hint`. If a string __doesn't belong__ to any section, then you should follow the rules below:
